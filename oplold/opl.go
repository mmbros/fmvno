...
// OPL type
type OPL struct {
	MaxNumSpedAlGiorno int                    `json:"-"` // must be >= 0
	MinNumSpedAlGiorno int                    `json:"-"` // must be >= 0
	NumSpedAlGiorno    util.RandIntByDateFunc `json:"-"`
	GiorniLavConsegne  util.RandIntByDateFunc `json:"-"`
	EsitoConsegne      util.RandIntByDateFunc `json:"-"`
	CalSpedizioni      util.Calendar          `json:"-"`
	Storico            SpedizioneMap          `json:"storico"`
	RespChan           chan *Spedizione
...
type SpedRequest struct {
	RequestID int
	Mobiles   model.MobileList
}

/*
// SpedResponse ...
type SpedResponse struct {
	sped *Spedizione
}
*/
...
func (opl *OPL) HandleRichiestaSpedizioni(requests <-chan SpedRequest) {
	for req := range requests {
		...
		sped := opl.addRequest(&req)
		opl.RespChan <- sped
	}
	opl.reqChanClosed = true
}

// addRequest create a new Spedizione and add to SpedizioniStorico and listSpedDaInviare
// gestisce (o almeno dovrebbe ...) l'accesso concorrente alle risorse
// - opl.listSpedDaInviare
func (opl *OPL) addRequest(req *SpedRequest) *Spedizione {

	// crea la nuova spedizione
	sped := newSpedizione(req.RequestID, req.Mobiles)
	if sped.Status == SpedizioneErrore {
		return sped
	}

	// aggiunge la nuova spedizione allo storico
	opl.Storico[sped.ID] = sped
